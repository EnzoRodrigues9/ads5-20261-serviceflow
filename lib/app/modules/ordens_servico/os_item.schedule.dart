...
class OsItemSchedule
    extends BaseSchedule<OsItem, OsItemRepository, OsItemProvider> {
  static final OsItemSchedule _instance = OsItemSchedule._init();
  ...
  Future<bool> syncByOsId(
    ...
    try {
      ...
      final itens = await repository.findByOsId(
        osId,
      );

      print('📦 Total itens encontrados: ${itens.length}');

      final pendingItens = itens
          .where(
            (i) => i.isSync == 0,
          )
          .toList();
      ...
      return successCount == pendingItens.length;
    } catch (e, stack) {
